...
def test_docker_compose_devcontainer_dind_settings():
    """Verify docker-compose.devcontainer.yml implements Docker socket mounting for DinD."""
    root_dir = Path(__file__).parent.parent.parent
    ...
    volumes = workspace_service.get("volumes", [])
    
    # Check for Docker socket mount
    has_docker_socket = any(
        "${DOCKER_SOCKET}:/var/run/docker.sock" in v or "/var/run/docker.sock:/var/run/docker.sock" in v
        for v in volumes
    )
    assert has_docker_socket, "Workspace service should mount Docker socket for DinD"

def test_docker_compose_backend_tls_env_support():
    """Verify docker-compose.yml (referenced by devcontainer) supports Docker TLS env vars."""
    root_dir = Path(__file__).parent.parent.parent
    ...
    # Convert list of "KEY=VALUE" to dict if necessary
    env_dict = {}
    ...
    # Check for DOCKER_HOST and other relevant env vars
    assert "DOCKER_HOST" in env_dict, "Backend should have DOCKER_HOST environment variable"
