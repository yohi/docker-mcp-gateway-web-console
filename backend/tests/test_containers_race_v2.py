"""Tests for container API race conditions (V2)."""
...
from app.services.containers import ContainerService, ContainerAlreadyExistsError

client = TestClient(app)

class TestContainerRaceV2:
    """Test suite for container API race conditions using the new architecture."""
    ...
    def test_create_container_already_exists_race(self):
        ...
        mock_container_service.create_container_with_auth.side_effect = ContainerAlreadyExistsError(
        ...
        assert "existing-container" in response.json()["detail"]
    ...
    def test_list_containers_unavailable_race(self):
        ...
        mock_container_service = AsyncMock(spec=ContainerService)
        mock_container_service.list_containers_with_auth.side_effect = ContainerUnavailableError(
