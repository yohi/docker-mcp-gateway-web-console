...
- バックエンド:
  ...
  - カタログ: `CATALOG_CACHE_TTL_SECONDS`, `CATALOG_DEFAULT_URL`, `CATALOG_OFFICIAL_URL`, `CATALOG_OFFICIAL_MAX_PAGES`, `GITHUB_TOKEN` (フォールバック用)。**トークン優先順位**: 保存された資格情報 (Bitwarden/DB) > UI設定トークン > 環境変数 GITHUB_TOKEN
