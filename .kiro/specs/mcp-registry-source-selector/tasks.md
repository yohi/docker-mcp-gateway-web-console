...
- [x] 4. カタログAPI エンドポイントを拡張する
- [x] 4.1 sourceパラメータを受け付けてソースIDを解決する機能を追加する
...
- [x] 6. 検索APIエンドポイントのsourceパラメータ対応を追加する
- [x] 6.1 検索機能でもソース選択を可能にする
...
- [x] 7. カタログソース選択UIコンポーネントを作成する
- [x] 7.1 ソース選択セレクタコンポーネントを実装する
...
- [x] 8. カタログページでソース切替を管理する
- [x] 8.1 ソース状態管理とリスト再取得の連携を実装する
...
- [x] 9. カタログ一覧表示のエラーハンドリングを拡張する
- [x] 9.1 構造化エラーコードに基づくエラー表示を実装する
...
- [x] 10. カタログAPI クライアントを拡張する
- [x] 10.1 sourceパラメータをサポートする
...
- [x] 11. バックエンドの単体テストを実装する
- [x] 11.1 ソース識別子と列挙型のバリデーションをテストする
...
- [x] 12.3 sourceパラメータ省略時と無効値のエンドツーエンド動作をテストする
...
- [x] 13.2 カタログページのソース状態管理をテストする
  - 初期状態でDockerが選択されていることを確認する
  - ソース変更時にカタログ再取得が発生することを確認する
  - エラー時も選択ソースが保持されることを確認する（モックでエラーをシミュレート実装済み）
  - _Requirements: 1.2, 1.3, 4.5_
  - _Test file: frontend/__tests__/app/catalog/page.test.tsx (lines 209-302)_
