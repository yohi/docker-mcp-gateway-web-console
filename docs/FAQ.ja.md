...
はい、カタログスキーマに従ってサーバー定義を追加し、HTTP/HTTPSでアクセス可能な場所にホストすれば、カタログのURLを切り替えることで読み込むことができます。

### 「インストール」とは何を意味しますか？

- **Dockerサーバー**: 「インストール」はコンテナ作成を指します。`/api/containers/install` が呼ばれ、ローカルで実行できる状態になります。
- **リモートサーバー**: 「インストール」はローカルGatewayへの登録を指します。`/api/remote-servers` に登録したうえで、必要に応じて「OAuth接続」や「接続テスト」を行って利用開始します（コンテナは作成されません）。
